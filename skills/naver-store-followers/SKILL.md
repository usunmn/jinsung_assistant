...
Python은 프로젝트 루트의 가상환경(`.venv`)을 사용한다.
`<repo_root>`는 jinsung_assistant 폴더 경로, `<skill_dir>`는 이 SKILL.md가 위치한 디렉토리 경로로 치환할 것.

### 1단계: 관심고객수 수집

```bash
<repo_root>/.venv/Scripts/python <skill_dir>/scripts/scrape_followers.py \
...
```bash
<repo_root>/.venv/Scripts/python <skill_dir>/scripts/update_sheet.py \
